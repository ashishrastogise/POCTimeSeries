...
public class POCTestOptions {
	...
	int opsPerSecond = 1;
	int duration = 18000;
	String databaseName = "POC_TS_DB";
	String collectionName = "POC_TS_COLL";
	String bpDatabaseName = "POC_BP_DB";
	String bpCollectionName = "POC_BP_COLL";
	
	int numcollections = 1;
	int secondaryIdx = 0;
	int reportTime = 10;
	String logfile = null;
	long granularityInMS = 1000;
	/**
	...
	String connectionDetails = "mongodb+srv://mongoadmin:mongoadmin@cluster5.9hhhi.mongodb.net/myFirstDatabase";
	boolean fulltext;
	...
	POCTestOptions(String[] args) throws ParseException
	{
		...
		cliopt.addOption("tscoll","tscoll",true,"using time series collection or using bucket pattern for older versions");
		...
		if(cmd.hasOption("tscoll"))
		{
			tscoll = Boolean.parseBoolean(cmd.getOptionValue("tscoll"));
		}
	}
}
